...
export function FollowingList({ initialFollowing }: {
    ...
    const handleUnfollow = async (username: string) => {
        ...
        try {
            ...
            if (result.success) {
                // Keep the user in list but with follow button
                setButtonStates(prev => ({
                    ...prev,
                    [username]: { state: 'idle' }
                }))
    ...
    if (followingList.length === 0) {
        return (
            <div className="text-center py-8 text-dimmed">
                ...
                <p className="text-sm mt-1">Find users to follow by browsing their profiles.</p>
            </div>
        )
    }
